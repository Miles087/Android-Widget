...
public class SuperCalendar extends Activity {
    ...
    private void initMonth(int iweekday, int iEndDay) {
        ...
        List<HasBellDate> list = new ArrayList<>();
        for (int i = 1; i < 50; i++) {
            if (iweekday == 7) {
                if (i <= 7) {
                    if (i == 1) {
                        list.add(new HasBellDate("周7"));
                    } else {
                        list.add(new HasBellDate("周" + (i - 1)));
                    }
                } else if (i > iweekday + 7 && i <= iweekday + 7 + iEndDay) {
                    list.add(new HasBellDate(i - 7 - iweekday + ""));
                }
            } else {
                if (i <= 7) {
                    if (i == 1) {
    ...
    class DateAdapter extends BaseAdapter {
        ...
        private List<HasBellDate> dateList;
        private Context mContext;

        DateAdapter(Context c, List<HasBellDate> list) {
            dateList = list;
            mContext = c;
        }
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            final HasBellDate item = dateList.get(i);
            String strDate = "";
